...
이 프로젝트는 **ABYSS SURVIVOR가 이미 완전히 구현된 상태**입니다.

> ⚠️ **절대 중요**: 이 템플릿은 빈 보일러플레이트가 아닙니다. 심해 배경의 Deep Diver가 돌연변이 해양 생물을 물리치는 서바이버 게임이 이미 완성되어 있습니다.
...
4. **CrossRamp 배포 (필수 — 절대 생략 금지)**:
   ...
   - `.crossramp` `asset_keys`가 `server.js`의 `$asset.mint('pearl')` 키와 일치하는지 확인
