    ...
    <div className="relative w-full h-screen overflow-hidden bg-black font-sans text-white">
        ...
        >
          <div className="absolute top-0 left-0 right-0 p-4 pointer-events-none">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-4 flex flex-col gap-2 items-center pointer-events-auto hud-ui z-20">
              <div className={`flex items-center gap-2 bg-black/60 backdrop-blur-sm px-6 py-2 rounded-full border border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.2)] ${goldBounce ? 'gold-bounce' : ''}`}>
      ...
      {isLevelUp && gameState === 'PLAYING' && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md px-4">
          <h2
            className="text-6xl md:text-8xl font-black mb-10 drop-shadow-[0_0_20px_rgba(6,182,212,0.8)] animate-bounce text-center italic"
      ...
      {gameState === 'TITLE' && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center px-4 bg-black/40 backdrop-blur-sm">
          {/* Total Gold Display */}
          <div className="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/60 backdrop-blur-sm px-6 py-2 md:px-8 md:py-3 rounded-full border border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.2)]">
            <Wallet className="w-5 h-5 md:w-7 md:h-7 text-cyan-400" />
            <span className="text-cyan-400 font-black text-xl md:text-2xl">{assets?.pearl || 0}</span>
          </div>
          ...
          <div className="flex flex-col items-center gap-4 md:gap-6 w-full max-w-xs md:max-w-none">
            ...
            >
              ...
              <button
              onClick={openShop}
              className="flex items-center gap-3 px-6 md:px-8 py-2 md:py-3 bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-400 hover:to-cyan-500 text-black font-extrabold text-lg md:text-xl rounded-full shadow-[0_0_20px_rgba(6,182,212,0.5)] transition-all transform hover:scale-105 active:scale-95 uppercase tracking-wider w-full justify-center"
            >
      ...
      {gameState === 'GAMEOVER' && (
        <div className={`absolute inset-0 z-20 flex flex-col items-center justify-center px-4 ${GAME_CONFIG.ui.gameOverBgClass} backdrop-blur-md`}>
          ...
          <div
            className="flex items-center gap-2 md:gap-3 mb-6 md:mb-12 text-lg md:text-2xl font-bold bg-black/50 px-4 md:px-8 py-2 md:py-4 rounded-full border shadow-[0_0_15px_rgba(6,182,212,0.2)]"
