...
export class MainScene extends Phaser.Scene {
  ...
  private orbitProjectiles: Phaser.Physics.Arcade.Sprite[] = [];
  ...
  private refreshOrbits() {
    ...
    for (let i = 0; i < this.orbitCount; i++) {
      const p = this.physics.add.sprite(this.player.x, this.player.y, GAME_CONFIG.currency.spriteKey);
      p.setDisplaySize(24, 24);
      p.setTint(0xffaaff);
      p.setData('damage', this.orbitDamage);
      p.setData('isOrbit', true);
      this.orbitProjectiles.push(p);
  ...
  update(time: number) {
    ...
    if (this.orbitCount > 0) {
      ...
      const radius = 80;
      for (let i = 0; i < this.orbitCount; i++) {
        const p = this.orbitProjectiles[i];
        if (p && p.active) {
