...
import { Gem, Wallet, Play, RotateCcw, Heart, LogOut, X, Check } from "lucide-react";
  ...
  useEffect(() => {
    const handleAddGold = () => {
      ...
      if (connected && server) {
        ...
        server.remoteFunction('addGold', [1], { throttle: 500, throttleKey: 'crystal' });
      }
    };
    ...
    <div className="relative w-full h-screen overflow-hidden bg-black font-sans text-white">
        ...
        >
          <div className="absolute top-0 left-0 right-0 p-4 pointer-events-none">
            ...
            <div className="absolute left-4 top-4 flex flex-col gap-2 pointer-events-auto hud-ui z-20">
              <div className="flex items-center gap-1 bg-black/60 backdrop-blur-sm px-4 py-2 rounded-full border border-white/10">
                {[...Array(health.max)].map((_, i) => (
                  <div 
                    key={i} 
                    className={`w-6 h-6 rounded-full transition-all duration-300 ${i < health.current ? 'bg-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.8)] scale-100' : 'bg-gray-800 border border-gray-600 scale-90'}`} 
                  />
                ))}
              </div>
            </div>
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-4 flex flex-col gap-2 items-center pointer-events-auto hud-ui z-20">
              <div className={`flex items-center gap-2 bg-black/60 backdrop-blur-sm px-6 py-2 rounded-full border border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.2)] ${goldBounce ? 'gold-bounce' : ''}`}>
                <Gem className="w-6 h-6 text-purple-400" />
                <span className="text-purple-400 font-bold text-2xl">{sessionGold}</span>
              </div>
            </div>
      ...
      {gameState === 'TITLE' && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center px-4 bg-black/40 backdrop-blur-sm">
          {/* Total Gold Display */}
          <div className="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/60 backdrop-blur-sm px-6 py-2 md:px-8 md:py-3 rounded-full border border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.2)]">
            <Wallet className="w-5 h-5 md:w-7 md:h-7 text-purple-400" />
            <span className="text-purple-400 font-black text-xl md:text-2xl">{assets?.crystal || 0}</span>
          </div>
      ...
      {gameState === 'GAMEOVER' && (
        <div className={`absolute inset-0 z-20 flex flex-col items-center justify-center px-4 ${GAME_CONFIG.ui.gameOverBgClass} backdrop-blur-md`}>
          ...
          <div
            className="flex items-center gap-2 md:gap-3 mb-6 md:mb-12 text-lg md:text-2xl font-bold bg-black/50 px-4 md:px-8 py-2 md:py-4 rounded-full border shadow-[0_0_15px_rgba(168,85,247,0.2)]"
