...
const Player = ({ position }: PlayerProps) => {
  ...
  // Mining state
  const isHacking = useHackingStore((s) => s.hackingTargetId !== null);
  ...
  const updatePlayerState = useCallback((): void => {
    ...
    // Mining state - play attack animation while in mining range
    // Movement is allowed; the range check in LootManager handles cancellation
    if (isHacking) {
  ...
  }, [isControlLocked, canInterrupt, lockControls, getCharacterMovementState, toCharacterState, getAnimation, setAnimation, getPlayerAction, isHacking]);
