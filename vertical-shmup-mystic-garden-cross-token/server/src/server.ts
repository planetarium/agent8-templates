export class Server {
  ...
  async getGameConfig(): Promise<{ exchangeRate: number; tokenSymbol: string; collectibleName: string }> {
    return {
      ...
      tokenSymbol: 'MGT',     // On-chain token symbol shown in WalletOverlay
      collectibleName: 'PETALS', // In-game collectible name shown in WalletOverlay
    };
  }

  // Award petals after defeating pests
  async claimPetals(amount: number): Promise<{ balance: number }> {
    if (amount <= 0 || amount > 1000) throw new Error('Invalid petal amount');
    await $asset.mint('petals', amount);
    const balance = await $asset.get('petals');
    return { balance };
  }

  // Get current petal balance
  async getPetalBalance(): Promise<{ balance: number }> {
    const balance = await $asset.get('petals');
    return { balance };
  }
  ...
  // Get player stats
  async getPlayerStats(): Promise<{ bestScore: number; bestWave: number; petals: number }> {
    const myState = await $global.getMyState();
    const petals = await $asset.get('petals');
    return {
