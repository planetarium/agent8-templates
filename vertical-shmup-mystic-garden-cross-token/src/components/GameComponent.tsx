...
const GameComponent: React.FC = () => {
  ...
  // ── Claim petals after game over ──────────────────────────
  const claimPetals = useCallback(async (amount: number) => {
    if (!connected || !server || amount <= 0 || claimPendingRef.current) return;
    claimPendingRef.current = true;
    try {
      const result = await server.remoteFunction('claimPetals', [amount]);
      if (result?.balance !== undefined) {
        EventBus.emit(EVENTS.PETAL_BALANCE, { balance: result.balance });
      }
    } catch (error) {
      console.error('Failed to claim petals:', error);
    } finally {
  ...
  useEffect(() => {
    ...
    const onGameOver = (data: import('../game/EventBus').GameOverData) => {
      ...
      if (data?.petalsPending > 0) {
        claimPetals(data.petalsPending);
      }
    };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden', background: '#0a1a0a' }}>
      {/* Phaser canvas */}
      <div
        id="phaser-game"
      ...
      {currentScene === 'WalletScene' && (
        <WalletOverlay
          ...
          tokenSymbol={gameConfig?.tokenSymbol ?? 'MGT'}
          collectibleName={gameConfig?.collectibleName ?? 'PETALS'}
        />
      )}
