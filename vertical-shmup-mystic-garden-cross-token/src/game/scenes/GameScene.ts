...
export class GameScene extends Phaser.Scene {
  ...
  private petalsCollected = 0;
  private petalsPending = 0;
  ...
  create() {
    ...
    this.joystickBaseRing = this.add.circle(0, 0, this.joystickRadius, 0xffffff, 0)
      .setDepth(20).setVisible(false);
    this.joystickBaseRing.setStrokeStyle(2, 0x88cc55, 0.35);
    this.joystickHandle = this.add.circle(0, 0, this.joystickHandleRadius, 0x88cc55, 0.25)
      .setDepth(21).setVisible(false);
    this.joystickHandle.setStrokeStyle(2, 0xaaee66, 0.8);
    ...
    // Camera flash-in
    this.cameras.main.flash(400, 20, 40, 10);
  }
  ...
  private spawnWave() {
    ...
    if (isBossWave) {
      ...
      this.showWaveBanner(`MOTH QUEEN WAVE ${this.waveNumber}`, '#cc44aa');
    } else {
      ...
      this.showWaveBanner(`WAVE ${this.waveNumber}`, '#66cc33');
    }
  }
  ...
  private fireBullet() {
    const b = this.add.rectangle(
      this.player.x, this.player.y - 36, 5, 18, 0x66cc33
    ).setDepth(4);
  ...
  private fireEnemyBullet(enemy: Enemy) {
    ...
    const b = this.add.ellipse(enemy.sprite.x, enemy.sprite.y, 10, 14,
      enemy.isBoss ? 0x9933cc : 0x882288
    ).setDepth(4);
    b.setData('vx', vx);
    b.setData('vy', vy);
  ...
  private spawnDrop(x: number, y: number, isBoss = false) {
    // Boss drops always petals; normal: 80% petal, 12% seed, 8% life
    const roll = Math.random();
    const type: 'petal' | 'seed' | 'life' = isBoss
      ? 'petal'
      : roll < 0.80 ? 'petal' : roll < 0.92 ? 'seed' : 'life';

    const key = type === 'petal' ? 'petal' : 'seed';
    const tint = type === 'life' ? 0x44ff44 : 0xffffff;
    const sprite = this.add.image(x, y, key);
    const size = type === 'petal' ? 30 : 22;
  ...
  private explodeAt(x: number, y: number, big = false) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const colors = [0x44aa22, 0x88dd44, 0xaaff66, 0xffcc44];
      ...
      for (let j = this.playerBullets.length - 1; j >= 0; j--) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) < hitR) {
          ...
          if (e.hp <= 0) {
            ...
            // Boss drops 15-20 petals, normal enemies drop 1-4
    ...
    for (let i = this.drops.length - 1; i >= 0; i--) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) < 36) {
        if (d.type === 'petal') {
          ...
          this.petalsPending++;
          this.showFloatText(d.sprite.x, d.sprite.y, '+1 petal', '#ff88aa');
        } else if (d.type === 'seed') {
          // Seed gives petals AND bonus score
          this.petalsCollected += 3;
          this.petalsPending += 3;
          this.score += 150;
          this.showFloatText(d.sprite.x, d.sprite.y, '+3 petals', '#ffcc44');
        } else {
          if (this.playerHP < this.maxHP) {
            this.playerHP++;
          }
          this.showFloatText(d.sprite.x, d.sprite.y, '+LIFE', '#66ff66');
        }
        this.emitHUD();
        d.sprite.destroy();
  ...
  private hitPlayer() {
    ...
    this.emitHUD();
    this.cameras.main.flash(120, 80, 20, 0);
  ...
  private endGame() {
    ...
    if (this.score > best) localStorage.setItem('mysticgarden_best', String(this.score));

    this.scene.start('GameOverScene', {
      score: this.score,
      wave: this.waveNumber,
      petals: this.petalsCollected,
      petalsPending: this.petalsPending,
    });
  }

  getPetals() { return this.petalsCollected; }
}
