...
- Top-level screen routing lives only in `App.tsx`. New screens extend the `renderContent()` switch and must not call `joinRoom` / `leaveRoom` from `useGameServer()` elsewhere.
...
- Server contract is implicit. The client calls remote functions `createRoom`, `setNickname`, `toggleReady`, `updateMyState`, `sendEffectEvent`, `applyDamage`, `revive`, `handlePing`, relies on the `onRoomCreate` / `onRoomJoin` / `onRoomLeave` hooks for room setup and teardown, and expects `UserState` shape with `stats.maxHp` / `stats.currentHp` — the server implementation must honor these. Room entry itself is the SDK's `joinRoom`, which is what fires `onRoomJoin`.
