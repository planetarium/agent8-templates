...
// Ember relic warm colour palette
...
const PROGRESS_UPDATE_THRESHOLD = 0.02;

// ─────────────────────────────────────────────
// RelicItem – individual ember relic mesh
// ─────────────────────────────────────────────
...
const RelicItem: React.FC<RelicItemProps> = React.memo(({ loot, isBeingMined }) => {
  const relicModel = useGLTF(Assets.objects.ember_crystal.url);
  ...
  useFrame((_, delta) => {
    ...
    if (matRef.current) {
      ...
      matRef.current.emissiveIntensity = isBeingMined ? 2.0 + pulse * 3.0 : 0.8 + pulse * 1.2;
    }
    if (groupRef.current) {
...
RelicItem.displayName = 'RelicItem';
...
const LootManager: React.FC<LootManagerProps> = ({ terrainData }) => {
  ...
  const handleLootCollected = useCallback(
    async (id: string) => {
      ...
      if (connected && server) {
        try {
          await server.remoteFunction('collectRelic', [id]);
        } catch (err) {
          console.error('Failed to collect relic on server', err);
        }
      }
    },
    [collectRelic, connected, server],
  );
