...
export class GameOverScene extends Phaser.Scene {
  ...
  create(data: { score: number; wave: number; stardust: number; stardustPending: number }) {
    ...
    if (data.score > best && data.score > 0) {
      localStorage.setItem('starblaster_best', String(data.score));
    }
    ...
    EventBus.emit(EVENTS.GAME_OVER, {
      ...
      stardustPending: data.stardustPending,
    });
