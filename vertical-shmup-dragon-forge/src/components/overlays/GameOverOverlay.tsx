    ...
    <div className={`${styles.root} ${styles.visible}`}>
      ...
      <div className={styles.panel}>
        ...
        <div className={styles.header}>
          <div className={styles.title}>DEFEATED</div>
        ...
        <div className={styles.stats}>
          ...
          <div className={styles.statRow}>
            <span className={styles.statLabel}>EMBERS</span>
            <span className={`${styles.statValue} ${styles.statFire}`}>
              <span className={styles.emberIcon}>&#x1F525;</span> {data.stardust}
            </span>
          </div>

          {data.stardust > 0 && (
            <div className={styles.exchangeHint}>
              {data.stardust} embers = {Math.floor(data.stardust / 10)} DFT token{data.stardust >= 10 ? 's' : ''}
            </div>
          )}
        </div>
