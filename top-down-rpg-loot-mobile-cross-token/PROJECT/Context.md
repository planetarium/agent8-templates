...
This is a **top-down mobile RPG dungeon-crawler loot game** boilerplate. The current default theme is "Crystal Dungeon" (dungeon knight vs monsters) — this is placeholder content only. Every new game built from this template must replace the concept entirely.
...
2. **Replace all placeholder content** — game name, sprites, enemies, boss, projectiles, UI theme, floor design
...
Phaser (scenes)
  ...
  ├── Player movement (virtual joystick + keyboard arrows)
  ├── Auto-attack to nearest enemy (magic projectiles)
  └── Enemy waves per floor, boss every 5th floor, gem drops
```
...
- Phaser 3.90 (Arcade Physics, top-down, 0 gravity)
- Server: @agent8/gameserver (structured TypeScript in server/)
...
- Asset key for in-game currency: `gem` (configurable per concept)
- CrossRamp exchange ratio configurable per game (e.g. 100 gems → 1 token)
- Game is mobile-first (virtual joystick, touch controls)
- Gravity set to 0,0 for top-down movement
- `getCrossRampShopUrl` called from React layer (GameComponent), triggered by Phaser WalletScene event
- Player auto-attacks nearest enemy — no manual attack button needed
- Room boundary enforced in GameScene (roomLeft/Right/Top/Bottom)
