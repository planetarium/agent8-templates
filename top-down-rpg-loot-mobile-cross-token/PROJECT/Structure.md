...
This project is a boilerplate for top-down mobile RPG dungeon-crawler loot games with CROSS blockchain token integration. Built with Phaser 3 and React overlays, it includes top-down dungeon room gameplay, virtual joystick movement, auto-attack combat, floor-based enemy waves with boss floors, gem collectibles with rarity tiers, and a Wallet scene for token exchange. An AI agent can receive a game concept and fully recreate the visual theme, overlays, and token branding — all without touching the core engine.
...
│       ├── TitleScene.ts                [CHANGE if theme] Panning BG + hero animation
│       ├── GameScene.ts                 [CHANGE] Core gameplay — enemies, floors, drops, boss
│       ├── GameOverScene.ts             [DO NOT MODIFY] Dark overlay → emits SCENE_CHANGE + GAME_OVER
...
- Player bullets: Ellipse primitive — require setDisplaySize()
- HP bars: Rectangle primitives attached to each enemy, positions updated every frame
- Room boundary: `roomLeft`, `roomRight`, `roomTop`, `roomBottom` define the playable dungeon area
