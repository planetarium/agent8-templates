...
| `HUD_UPDATE` | Phaser → React | payload: HUDData — score, floor, gems, hp |
| `GAME_OVER` | Phaser → React | payload: GameOverData — game over stats |
| `OPEN_CROSS_RAMP` | Phaser → React | Trigger CROSS Mini Hub open |
| `GEM_BALANCE` | React → React | payload: { balance: number } — after claimGems |
...
- **Required prompt fields** — every generation call must include all four:
  ...
  - `details` — subject description, pose, expression, costume, key visual features. All characters must be top-down view.
...
- **Required image assets**: `background` (dungeon floor tileable), `player` (top-down hero), each `enemy` type (top-down), `boss` (top-down), `gem` (common collectible), `rareGem` (rare collectible), `epicGem` (epic collectible), `playerBullet` (projectile).
...
- **Room bounds**: player and enemies must stay within `roomLeft/Right/Top/Bottom`. Use `Phaser.Math.Clamp()`.
- **Top-down orientation**: all character sprites must face appropriate directions. Player rotates to face nearest enemy.

## Core Gameplay (Template Default)

- [x] Top-down dungeon room (bounded play area with wall border)
- [x] Virtual joystick movement (all 4 directions)
- [x] Arrow key support (desktop)
- [x] Auto-attack: player shoots toward nearest enemy
- [x] Enemy waves per floor that escalate in difficulty
- [x] Boss floor every 5th floor with melee + orbit movement
- [x] Player HP with invincibility frames
- [x] Gem drops from enemies (common +1, rare +5, epic +20)
- [x] HP life restore drops (rare)
- [x] Score system with local best tracking
- [x] Particle explosion effects
- [x] Floor-clear progression with delay
...
```
GameScene.ts          → gemsPending aggregation → GAME_OVER emit
GameComponent.tsx     → claimGems(amount) → server.remoteFunction('claimGems', [amount])
server/src/server.ts  → $asset.mint('gem', amount)   ← asset key
WalletOverlay.tsx     → "100 GEMS = 1 GDT"           ← token name / exchange rate display
```
...
|--------|-------------|-------|
| `server.ts` — `$asset.mint('gem')` asset key | Yes | Must change **in sync** with GameScene drop type `'gem'` string |
| `server.ts` — `$asset.get('gem')` | Yes | Use same key as mint |
| `server.ts` — function names `claimGems`, `getGemBalance` | **No** | GameComponent calls `remoteFunction('claimGems')` — hardcoded |
| `GameComponent.tsx` entire file | **No** | claimGems, openCrossRamp logic must never be modified |
| `EventBus.GEM_BALANCE` | **No** | Changing event key breaks integration |
| `WalletOverlay.tsx` — token name, exchange rate text | Yes | **Must** update to match concept |

### When Changing Exchange Rate

To change the exchange rate (e.g. 100 gems = 1 GDT):
1. `server.ts` — update claimGems internal logic (validation range, etc.)
2. `WalletOverlay.tsx` — update display text ("100 GEMS", "1 GDT TOKEN")
Both must be updated together so UI matches actual behavior.

### Template Default

- [x] CrossRamp deployed (project: set via `read_gameserver_sdk_crossramp`)
- [x] Token: GDT (100 gems = 1 GDT)
- [x] Asset key: `gem`
- [x] CROSS Mini Hub button in WalletScene
- [x] Server: claimGems mints gem asset
...
- Phaser gravity set to {x:0, y:0} for top-down physics
- Room boundary enforced via Phaser.Math.Clamp in GameScene
- Drops attract to player when within 80px radius
- HP bars use separate Rectangle objects, positions updated each frame
