...
이 프로젝트는 **CELESTIAL SCAVENGERS가 이미 완전히 구현된 상태**입니다.

> ⚠️ **절대 중요**: 이 템플릿은 빈 보일러플레이트가 아닙니다. 천상의 불사조 기체로 하늘의 석조 괴물들을 격파하는 수직 슈팅 게임이 이미 완성되어 있습니다.
...
4. **CrossRamp 배포 (필수 — 절대 생략 금지)**:
   ...
   - `.crossramp` `asset_keys`가 `server.js`의 `$asset.mint('shards')` 키와 일치하는지 확인

---

## 섹션 0-3 — 이 템플릿의 원래 목적 (참고)

이 템플릿의 기반 보일러플레이트는 **수직 슈팅(shmup) 2D 모바일 게임**이며, 같은 구조에서 다양한 테마의 게임을 만들 수 있습니다.

## 섹션 0-4 — 변경 가능한 것 (사용자 요청 시)
...
| 수집 아이템 | `src/assets.json` | items.shards (토큰명 변경 가능) |
