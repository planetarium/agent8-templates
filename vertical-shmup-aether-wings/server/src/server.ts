export class Server {
  ...
  async getGameConfig(): Promise<{ exchangeRate: number; tokenSymbol: string; collectibleName: string }> {
    return {
      ...
      tokenSymbol: 'GLOW',     // On-chain token symbol shown in WalletOverlay
      collectibleName: 'LUMINA', // In-game collectible name shown in WalletOverlay
    };
  }

  // Award lumina essence after defeating enemies
  async claimStardust(amount: number): Promise<{ balance: number }> {
    if (amount <= 0 || amount > 1000) throw new Error('Invalid lumina amount');
    await $asset.mint('lumina', amount);
    const balance = await $asset.get('lumina');
    return { balance };
  }

  // Get current lumina balance
  ...
  async getPlayerStats(): Promise<{ bestScore: number; bestWave: number; stardust: number }> {
    const myState = await $global.getMyState();
    const stardust = await $asset.get('lumina');
    return {
