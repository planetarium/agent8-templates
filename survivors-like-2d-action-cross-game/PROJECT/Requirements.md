...
## ⚠️ CRITICAL: This Is a Pre-Built Game — Do NOT Rebuild on First Prompt

> On the first prompt: **read the code, report what's implemented, wait for user instructions.**
> Do NOT generate assets, redesign UI, or rewrite config files automatically.

---

## CrossRamp Deployment (MANDATORY on First Prompt — Always)

> ⚠️ `.crossramp` and `.env` have been removed from this template. You **must always** deploy CrossRamp on the first prompt, regardless of user request.
...
## Asset + UI Changes (Only If User Requests)

> Assets and UI redesign are only done when the user explicitly asks. Do NOT automatically generate assets or redesign UI.
