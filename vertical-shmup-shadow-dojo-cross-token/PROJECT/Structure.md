# Project Structure

## 🚨 CROSSRAMP — FIRST PROMPT MANDATORY

> **NEVER skip CrossRamp deployment. Required on every first prompt without exception.**

- Call `read_gameserver_sdk_v2`
- Call `read_gameserver_sdk_crossramp`
- Verify `.crossramp` `asset_keys` matches server asset key (`stardust`)

---

## ⚠️ Pre-Built Game — Do NOT Recreate on First Prompt

**You MUST on first prompt:**
1. Write `.crossramp` file with token config
2. Call `read_gameserver_sdk_v2` then `read_gameserver_sdk_crossramp` → deploys CrossRamp
3. Read `server/src/server.ts` → confirm asset key matches `.crossramp`
4. Generate EXACTLY 2 images: drop item icon + token logo → update `assets.json`
5. Run `bun run build`

**You MUST NOT on first prompt:**
- Regenerate player ninja / enemy / boss / bullet / background images — game is already built
- Rewrite GameScene.ts, overlays, or config files

---

## Current Game: Shadow Dojo

| Element | Implementation |
|---------|---------------|
| Theme | Dark dojo vertical shooter — ninja warrior battles descending shadow enemies in an ember-lit arena |
| Player | Ninja warrior sprite, 3 HP, moves via virtual joystick |
| Enemies | Shadow enemy waves every 8 seconds; boss wave every 5th wave (40+ HP, grows with wave number) |
| Drop Item | Stardust souls (`soul`), crystals (`crystal`), life pickups (`life`) |
| Currency | `stardust` token · exchange rate 100 per unit from `getGameConfig()` |
| Score | Score increments per enemy kill; wave survival tracking; leaderboard via `saveHighScore()` |
| CrossRamp | NOT deployed — deploy on first prompt |

---

## File Map

```
src/
├── assets.json              [CHANGE] image URLs (player, enemy, boss, drop, bg)
├── App.tsx                  [DO NOT MODIFY]
├── game/
│   ├── Game.ts              [DO NOT MODIFY]
│   ├── EventBus.ts          [DO NOT MODIFY]
│   └── scenes/
│       ├── BootScene.ts     [DO NOT MODIFY]
│       ├── TitleScene.ts    [CHANGE] title assets, ninja sprite
│       ├── GameScene.ts     [CHANGE] gameplay logic, enemy waves, drop system
│       ├── GameOverScene.ts [CHANGE]
│       └── WalletScene.ts   [DO NOT MODIFY]
└── components/
    ├── GameComponent.tsx    [DO NOT MODIFY]
    └── overlays/
        ├── TitleOverlay.tsx        [CHANGE] title text, branding
        ├── HUDOverlay.tsx          [CHANGE] HP display, score, stardust count
        ├── GameOverOverlay.tsx     [CHANGE] score summary, best score
        ├── WalletOverlay.tsx       [DO NOT MODIFY]
        ├── CrossRampOverlay.tsx    [DO NOT MODIFY]
        └── LoadingOverlay.tsx      [DO NOT MODIFY]
server/src/server.ts         [CHANGE] asset key rename, exchange rate
```

---

## Absolute Constraints

- NEVER use scaleX/scaleY in tweens → use displayWidth/displayHeight only
- ALWAYS call setDisplaySize() on every image/sprite
- EventBus event key names must NOT change
- Scene keys: BootScene, TitleScene, GameScene, GameOverScene, WalletScene — never rename

---

## CrossRamp: Currency Rename (if user requests)

3-way sync:
1. `server.ts` → `$asset.mint('새키', amount)` and all `$asset.get('stardust')` references
2. `GameScene.ts` → all references to old key
3. `.crossramp` → `asset_keys: ["새키"]`
