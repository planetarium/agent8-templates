...
const GameComponent: React.FC = () => {
  ...
  // ── Claim souls after game over ─────────────────────────────
  const claimSouls = useCallback(async (amount: number) => {
    ...
    try {
      ...
      if (result?.balance !== undefined) {
        EventBus.emit(EVENTS.SOUL_BALANCE, { balance: result.balance });
      }
    } catch (error) {
      console.error('Failed to claim souls:', error);
    } finally {
  ...
  useEffect(() => {
    ...
    const onGameOver = (data: import('../game/EventBus').GameOverData) => {
      ...
      if (data?.soulsPending > 0) {
        claimSouls(data.soulsPending);
      }
    };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden', background: '#0a0008' }}>
      {/* Phaser canvas */}
      <div
        id="phaser-game"
        ref={gameContainerRef}
        style={{ width: '100%', height: '100vh', background: '#0a0008' }}
      />
      ...
      {currentScene === 'WalletScene' && (
        <WalletOverlay
          ...
          collectibleName={gameConfig?.collectibleName ?? 'SOUL'}
        />
      )}
