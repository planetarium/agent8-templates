import Phaser from 'phaser';
import { EventBus, EVENTS } from '../EventBus';

export class GameOverScene extends Phaser.Scene {
  private bg!: Phaser.GameObjects.TileSprite;

  constructor() {
    super({ key: 'GameOverScene' });
  }

  create(data: { score: number; wave: number; souls: number; soulsPending: number }) {
    const W = this.cameras.main.width;
    const H = this.cameras.main.height;

    // Background only — UI is handled by React
    this.bg = this.add.tileSprite(W / 2, H / 2, W, H, 'bg');
    this.bg.setDisplaySize(W, H);
    this.add.rectangle(W / 2, H / 2, W, H, 0x0a0008, 0.7);

    // Save best score
    const best = parseInt(localStorage.getItem('shadowdojo_best') || '0', 10);
    if (data.score > best && data.score > 0) {
      localStorage.setItem('shadowdojo_best', String(data.score));
    }

    // Emit to React UI
    EventBus.emit(EVENTS.SCENE_CHANGE, { scene: 'GameOverScene' });
    EventBus.emit(EVENTS.GAME_OVER, {
      score: data.score,
      wave: data.wave,
      souls: data.souls,
      soulsPending: data.soulsPending,
    });

    this.cameras.main.fadeIn(400, 10, 0, 8);
  }

  /** Called by React overlay to navigate to another scene properly */
  goToScene(key: string, payload?: object) {
    this.cameras.main.fadeOut(300, 10, 0, 8);
    this.cameras.main.once('camerafadeoutcomplete', () => {
      this.scene.start(key, payload);
    });
  }

  update() {
    if (this.bg) this.bg.tilePositionY -= 0.5;
  }
}
