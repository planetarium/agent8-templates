...
// Starflower ice crystal colour palette
...
// ─────────────────────────────────────────────
// CrystalItem – individual starflower mesh
// ─────────────────────────────────────────────
...
const CrystalItem: React.FC<CrystalItemProps> = React.memo(({ loot, isBeingMined }) => {
  const crystalModel = useGLTF(Assets.objects.sapphire_crystal_cluster.url);
...
const LootManager: React.FC<LootManagerProps> = ({ terrainData }) => {
  ...
  const handleLootCollected = useCallback(
    async (id: string) => {
      ...
      if (connected && server) {
        try {
          await server.remoteFunction('collectStarflower', [id]);
        } catch (err) {
          console.error('Failed to collect starflower on server', err);
        }
      }
    },
    [collectFlower, connected, server],
  );
