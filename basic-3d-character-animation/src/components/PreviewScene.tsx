...
const PreviewScene: React.FC = () => {
  const [currentAction, setCurrentAction] = useState<CharacterAction>(CharacterAction.IDLE);
      ...
      >
        <Canvas camera={{ position: [0, 0, 5], fov: 50 }} shadows>
          ...
          <group scale={2} position={[0, -1.75, 0]}>
            <Character currentActionRef={currentActionRef} onAnimationComplete={onAnimationComplete} />
          </group>
        ...
        {Object.values(CharacterAction)
          ...
          .map((action) => (
            <button
              ...
              style={{
                padding: '8px 16px',
                backgroundColor: currentAction === action ? '#2980b9' : '#3498db',
                color: 'white',
                border: 'none',
                borderRadius: '4px',
                cursor: 'pointer',
