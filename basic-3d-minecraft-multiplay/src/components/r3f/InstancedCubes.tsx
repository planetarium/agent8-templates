...
export function InstancedCubes() {
  ...
  const chunks = useMemo(() => {
    ...
    return finalChunks;
  }, [cubes, activeChunks, camera, sortChunksByDistance, cameraChunkTrigger]);
      ...
      {activeChunksArray
        ...
        .map((chunk) => (
          <RigidBody key={chunk.key} type="fixed" colliders={false} userData={{ type: 'cubeChunk', chunkKey: chunk.key }}>
            ...
            {/* {chunk.cubes.map((cube, index) => (
              <CuboidCollider key={`${chunk.key}-${index}`} position={[cube.position[0], cube.position[1], cube.position[2]]} args={[0.5, 0.5, 0.5]} />
            ))} */}
          </RigidBody>
        ))}
    </>
