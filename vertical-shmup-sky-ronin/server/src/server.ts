export class Server {
  ...
  async getGameConfig(): Promise<{ exchangeRate: number; tokenSymbol: string; collectibleName: string }> {
    return {
      exchangeRate: 100,         // Must match exchange_rate in .crossramp — 100 Spirit Orbs = 1 SRT
      tokenSymbol: 'SRT',        // Sky Ronin Token on-chain symbol shown in WalletOverlay
      collectibleName: 'SPIRIT ORBS', // In-game collectible name shown in WalletOverlay
    };
  }
  ...
  async claimStardust(amount: number): Promise<{ balance: number }> {
    ...
    await $asset.mint('stardust', amount);
    const balance = await $asset.get('stardust');
    return { balance };
  }
  ...
  async getPlayerStats(): Promise<{ bestScore: number; bestWave: number; stardust: number }> {
    const myState = await $global.getMyState();
    const stardust = await $asset.get('stardust');
    return {
