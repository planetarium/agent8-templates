...
이 프로젝트는 **Sky Ronin이 이미 완전히 구현된 상태**입니다.

> ⚠️ **절대 중요**: 이 템플릿은 빈 보일러플레이트가 아닙니다. 봉건 일본풍 수직 슈팅 모바일 게임(Spirit Orbs 수집 + CROSS 토큰 교환)이 이미 완성되어 있습니다.
...
4. **CrossRamp 배포 (필수 — 절대 생략 금지)**:
   ...
   - `.crossramp` `asset_keys`가 `server.js`의 `$asset.mint('stardust')` 키와 일치하는지 확인
