...
export class GameScene extends Phaser.Scene {
  ...
  private spawnWave() {
    ...
    if (isBossWave) {
      ...
      this.showWaveBanner(`⚠ ONI BOSS WAVE ${this.waveNumber} ⚠`, '#ff3300');
    } else {
      ...
      this.showWaveBanner(`⛩ WAVE ${this.waveNumber} ⛩`, '#e8b84b');
    }
  }
  ...
  private fireBullet() {
    const b = this.add.rectangle(
      this.player.x, this.player.y - 36, 4, 20, 0x7adfb0
    ).setDepth(4);
  ...
  private explodeAt(x: number, y: number, big = false) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const colors = [0xff4422, 0xe8b84b, 0xffd080, 0xc44b2a];
    ...
    for (let i = this.drops.length - 1; i >= 0; i--) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) < 36) {
        if (d.type === 'stardust') {
          ...
          this.showFloatText(d.sprite.x, d.sprite.y, '+1 ◉', '#7adfb0');
        } else if (d.type === 'crystal') {
          // Crystal gives spirit orbs AND bonus score
          this.stardustCollected += 3;
          this.stardustPending += 3;
          this.score += 150;
          this.showFloatText(d.sprite.x, d.sprite.y, '+3 ◉', '#aaffcc');
        } else {
  ...
  private endGame() {
    ...
    if (this.score > best) localStorage.setItem('skyronin_best', String(this.score));
