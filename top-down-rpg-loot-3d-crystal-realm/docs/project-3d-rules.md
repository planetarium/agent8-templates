...
> ⚠️ **절대 중요**: 이 템플릿은 빈 보일러플레이트가 아닙니다. 3D 탑다운 오픈월드 자원 수집 게임 REALM OF RELICS가 이미 완성되어 있습니다.
...
4. **CrossRamp 배포 (필수 — 절대 생략 금지)**:
   ...
   - `.crossramp` `asset_keys`가 `server/src/server.ts`의 `$asset.mint('crystal')` 키와 일치하는지 확인
