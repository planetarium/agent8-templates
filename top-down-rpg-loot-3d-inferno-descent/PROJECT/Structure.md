...
3. Deploy CrossRamp (MANDATORY — never skip):
   - Call `read_gameserver_sdk_v2`
   - Call `read_gameserver_sdk_crossramp`
   - Verify `.crossramp` `asset_keys` matches `server.ts` `$asset.mint('fire_soul')` key
...
## Current Game: Inferno Descent

| Element | Implementation |
|---------|---------------|
| Theme | Volcanic hellscape — demon hunter harvests fire souls from molten rock formations in an inferno realm |
| Collectible | Fire Soul (`fire_soul`) |
| Environment | seed: `"inferno-descent"`, volcanic terrain with burned ground, cracked magma, and volcanic rock textures |
| Loot Colors | Fiery orange (`#ff6600`), Deep crimson (`#cc2200`), Molten gold (`#ffaa00`) |
| Server Asset Key | `fire_soul` |
...
- `server.ts` function names `collectSoul` / `getMyAssets` must not be renamed (client hardcoded)
