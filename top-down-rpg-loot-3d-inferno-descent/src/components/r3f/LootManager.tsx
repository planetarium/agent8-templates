...
// Fire soul colour palette (pre-created Color objects to avoid GC)
const VARIANT_EMISSIVE_COLORS = [
  new Color('#ff6600'),
  new Color('#cc2200'),
  new Color('#ffaa00'),
];
...
const PROGRESS_UPDATE_THRESHOLD = 0.02; // less frequent store updates

// ─────────────────────────────────────────────
// SoulItem – individual fire soul mesh
// Optimised: no per-item pointLight, shared material approach
// ─────────────────────────────────────────────
...
const SoulItem: React.FC<SoulItemProps> = React.memo(({ loot, isBeingMined }) => {
  const soulModel = useGLTF(Assets.objects.molten_gold_rock.url);
  const matRef = useRef<MeshStandardMaterial | null>(null);
  const groupRef = useRef<THREE.Group>(null);
  const timeRef = useRef(Math.random() * Math.PI * 2);
  const emissiveColor = VARIANT_EMISSIVE_COLORS[loot.variant];
  ...
  useFrame((_, delta) => {
    const speed = isBeingMined ? 8 : 2.5;
    timeRef.current += delta * speed;
    if (matRef.current) {
      const pulse = 0.5 + Math.sin(timeRef.current) * 0.5;
      matRef.current.emissiveIntensity = isBeingMined ? 2.0 + pulse * 3.0 : 1.0 + pulse * 1.5;
    }
    if (groupRef.current) {
...
const LootManager: React.FC<LootManagerProps> = ({ terrainData }) => {
  ...
  const collectSoul = useInventoryStore((state) => state.collectSoul);
  const { config } = useQualityStore();
  const { connected, server } = useGameServer();
  const { miningTargetId, miningProgress, startMining, setMiningProgress, stopMining } =
    useMiningStore();

  // Quality-driven max souls
  const maxActive = config.maxSouls;
  ...
  const usedSlotsRef = useRef<Set<number>>(new Set(Array.from({ length: maxActive }, (_, i) => i)));
  ...
  const handleLootCollected = useCallback(
    async (id: string) => {
      ...
      if (connected && server) {
        try {
          await server.remoteFunction('collectSoul', [id]);
        } catch (err) {
          console.error('Failed to collect soul on server', err);
        }
      }
    },
    [collectSoul, connected, server],
  );
  ...
  // Optimised: single useFrame for proximity + mining + respawn
  useFrame((_, delta) => {
    ...
    // 1. Respawn – cap at maxActive
    const queue = respawnQueueRef.current;
    let i = 0;
    while (i < queue.length) {
      if (now >= queue[i].readyAt && activeLootRef.current.length < maxActive) {
        queue.splice(i, 1);
    ...
    // 2. Find closest soul (use squared distance – avoid sqrt)
    let closestId: string | null = null;
    let closestDistSq = MINING_TRIGGER_RADIUS * MINING_TRIGGER_RADIUS;
    ...
    if (currentTarget) {
      ...
      if (distSq > MINING_CANCEL_RADIUS * MINING_CANCEL_RADIUS) {
        stopMining();
        miningTargetIdRef.current = null;
        miningTimerRef.current = 0;
        return;
