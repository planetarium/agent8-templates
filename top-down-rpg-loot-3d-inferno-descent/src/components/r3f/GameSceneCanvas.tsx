 ...
 *
 * Optimised: memoized gl config, reduced shadow frustum
 */
const GameSceneCanvas = () => {
  ...
  const glConfig = useMemo(() => ({
    ...
    // Reduce default buffer precision for perf
    depth: true,
    stencil: false,
    alpha: false,
  }), [config.antialias]);
  ...
  return (
    <Canvas
      ...
      dpr={config.dpr}
      gl={glConfig}
      // Performance: limit frame loop to 60fps max
      frameloop="always"
      // Reduce camera far plane to help with depth buffer precision
    ...
    >
      <Physics paused={!isMapPhysicsReady}>
        <Suspense fallback={null}>
          ...
          {!config.shadows && <directionalLight position={[40, 80, 20]} intensity={1.8} color="#ffaa77" />}
