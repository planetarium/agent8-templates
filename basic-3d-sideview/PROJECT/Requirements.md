# Requirements — basic-3d-sideview (Incanto)
...
- ATTACK button = punch (KeyF injection): one-shot mixamo clip through
  `animationLoop:false` + the `animationFinished` signal, locking movement
  animation switches while acting.
