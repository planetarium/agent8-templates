# Structure — basic-3d-sideview (Incanto)
...
physics auto, keyboard, `pointer: { lockOnClick: false }` — fixed camera,
Mouse0 = punch, renderer, start) → button key-injection → `window.game`.
