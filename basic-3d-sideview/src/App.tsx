/**
 * basic-3d-sideview on Incanto: a Mario-style ribbon of platforms seen
 * side-on (fixed +z camera, fov 60), A/D or arrows to run, Space to jump
 * gaps, F or left click to punch (one-shot animation with input lock).
 */
...
export function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // physics (auto) → keyboard + pointer → renderer → start.
      const game = await createGame3D({
        debug: import.meta.env.VITE_INCANTO_DEBUG === '1',
        canvas,
        scene: sceneJson,
        behaviors: { FollowLight, CharacterAnimator },
        pointer: { lockOnClick: false }, // no pointer lock: fixed side camera; Mouse0 = punch
      });
