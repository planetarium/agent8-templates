...
8. **`src/App.tsx`** — UI 전면 재설계 (레이아웃, 색상, CSS, 아이콘, 폰트, 배경 효과) — **첫 프롬프트에서 Step 2와 함께 완료. 미루지 말 것.** **크로스코인 연동 반드시 보존**: `useGameServer`/`useAsset` 훅, `handleAddGold`(server.remoteFunction), `openShop`(getCrossRampShopUrl), CROSS Exchange 버튼, 전체 골드 잔액 표시.
...
### 크로스코인 연동 필수 보존 (App.tsx 재설계 시)

App.tsx를 재설계할 때 아래 요소를 **반드시 유지**해야 한다. 하나라도 누락 시 불완전한 구현.

- `useGameServer()`, `useAsset()` 훅
- `handleAddGold` 내 `server.remoteFunction('addGold', [1], { throttle: 500, throttleKey: 'gold' })`
- `openShop` 내 `server.getCrossRampShopUrl("en")` + `window.open`
- 타이틀 화면: 전체 골드 잔액 (`assets?.gold`)
- 타이틀 화면: **CROSS Exchange 버튼** (openShop 호출)
- 플레이 중 HUD: 세션 골드 표시
- 게임오버: 세션 골드 결산 표시
...
- **전체 골드 잔액 표시** (`assets?.gold`) — 크로스코인 연동 필수
- **CROSS Exchange 버튼** — 상점 열기, openShop 호출
