...
**블록체인 키워드 ≠ 게임 컨셉**: `CROSS`, `chain`, `token`, `blockchain`, `wallet` 등은 **블록체인 기술 스펙**이며 게임 세계관 결정에 사용하지 않는다. 유저 프롬프트에 이 키워드만 있고 세계관이 명시되지 않았다면 → **1-B로 이동**하여 조합 테이블에서 자유롭게 선택.
...
**중요**: 매 생성마다 반드시 **다른 세계관**을 선택하라. 사이버펑크/SF는 다른 세계관과 **동등한 선택지**이며 기본값이 아니다. 블록체인/토큰 관련 키워드가 있어도 세계관은 이 테이블에서 자유롭게 선택한다.
...
**조합 예시 (참고용 — 순서는 무관, 다양한 컨셉 골고루 활용)**  
- 해양 탐험 × 다이버 × 심해 생물 × 청록  
- 마법 숲 × 요정 × 버섯 몬스터 × 연두  
...
2. **에셋 생성** (이미지 생성 도구 사용): 플레이어, 적 타입별, 배경, XP 젬, 통화 아이콘  
   → **에셋 생성 직후 Step 8 App.tsx UI 재설계를 반드시 이어서 실행 (세트 작업). 에셋만 교체하고 UI를 그대로 두는 것은 허용되지 않는다.**
