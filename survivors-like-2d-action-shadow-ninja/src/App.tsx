...
import {
  Zap, Wallet, Play, RotateCcw, Heart, LogOut, X, Check, Sword,
} from "lucide-react";
...
function App() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black text-white" style={{ fontFamily: "'Noto Serif JP', serif" }}>
      {/* HUD - Playing State */}
      {gameState === 'PLAYING' && (
        <div
        ...
        >
          <div className="absolute top-0 left-0 right-0 p-4 pointer-events-none">
            ...
            <div className="absolute left-4 top-4 flex flex-col gap-2 pointer-events-auto hud-ui z-20">
              <div className="flex items-center gap-1 bg-black/80 backdrop-blur-sm px-4 py-2 rounded border border-red-800/50 shadow-[0_0_12px_rgba(220,38,38,0.2)]">
                {[...Array(health.max)].map((_, i) => (
                  <Heart
                    key={i}
                    className={`w-6 h-6 transition-all duration-300 ${
                      i < health.current
                        ? 'fill-red-500 text-red-500 drop-shadow-[0_0_8px_rgba(220,38,38,0.9)] scale-100'
            ...
            {/* Top Center: Demon Gold */}
            <div className="absolute left-1/2 -translate-x-1/2 top-4 flex flex-col gap-2 items-center pointer-events-auto hud-ui z-20">
              <div className={`flex items-center gap-2 bg-black/80 backdrop-blur-sm px-5 py-2 rounded border border-amber-700/50 shadow-[0_0_15px_rgba(217,119,6,0.25)] ${goldBounce ? 'scale-110' : ''} transition-transform duration-150`}>
                <Zap className="w-5 h-5 text-amber-400" />
                <span className="text-amber-400 font-bold text-xl tracking-widest">{sessionGold}</span>
                <span className="text-amber-700 text-xs uppercase tracking-widest">GOLD</span>
              </div>
            </div>
            ...
            <div className="absolute right-4 top-4 flex flex-col gap-2 pointer-events-auto hud-ui z-20">
              <button
                ...
                className="flex items-center justify-center bg-black/80 hover:bg-red-950/80 backdrop-blur-sm p-3 rounded border border-red-900/30 hover:border-red-500/50 transition-colors shadow-[0_0_10px_rgba(0,0,0,0.5)]"
              >
          ...
          <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 w-72 md:w-[560px] pointer-events-auto hud-ui z-20 ${xpPulse ? 'scale-105' : ''} transition-transform duration-150`}>
            <div className="flex items-center justify-between text-xs font-bold mb-2 px-1">
              <span className="text-red-400 tracking-widest uppercase" style={{ fontFamily: "'Noto Serif JP', serif" }}>LVL {xp.level}</span>
              <span className="text-red-300/70">{xp.current} / {xp.max} XP</span>
            </div>
            <div className="w-full bg-black/70 backdrop-blur-md rounded-sm h-3 border border-red-800/40 overflow-hidden shadow-[0_0_10px_rgba(0,0,0,0.5)]">
              <div
          ...
          {joystick?.active && !isLevelUp && (
            ...
            >
              {/* Base ring - crimson themed */}
              <div className="absolute inset-0 rounded-full border-2 border-red-500/40 bg-red-950/20 backdrop-blur-sm shadow-[0_0_20px_rgba(220,38,38,0.2)]" />
              {/* Stick */}
              <div
                ...
                style={{
                  ...
                  top: JOYSTICK_RADIUS,
                  background: 'radial-gradient(circle, rgba(220,38,38,0.9) 0%, rgba(220,38,38,0.3) 100%)',
      ...
      {isQuitModalOpen && gameState === 'PLAYING' && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-md px-4 pointer-events-auto hud-ui">
          <div className="bg-gray-950 border border-red-800/50 rounded-lg p-8 max-w-sm w-full flex flex-col items-center shadow-[0_0_60px_rgba(220,38,38,0.1)]">
            {/* Decorative top bar */}
            <div className="w-full h-px bg-gradient-to-r from-transparent via-red-700 to-transparent mb-6" />
            <LogOut className="w-14 h-14 text-red-500 mb-4 drop-shadow-[0_0_12px_rgba(220,38,38,0.7)]" />
            <h2 className="text-2xl font-black text-white mb-2 text-center tracking-widest uppercase" style={{ fontFamily: "'Noto Serif JP', serif" }}>{GAME_CONFIG.ui.quitConfirmTitle}</h2>
            <p className="text-gray-500 text-center mb-8 text-sm leading-relaxed">{GAME_CONFIG.ui.quitConfirmMessage}</p>

            <div className="flex gap-3 w-full">
              <button
                onClick={handleCloseQuitModal}
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-gray-900 hover:bg-gray-800 text-white font-bold rounded transition-colors border border-gray-700"
              >
                <X className="w-4 h-4" /> STAY
              </button>
              <button
                onClick={handleConfirmQuit}
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-red-900 hover:bg-red-800 text-white font-bold rounded transition-colors border border-red-700 shadow-[0_0_15px_rgba(220,38,38,0.3)]"
              >
                <Check className="w-4 h-4" /> RETREAT
              </button>
            </div>
            <div className="w-full h-px bg-gradient-to-r from-transparent via-red-700 to-transparent mt-6" />
          </div>
        </div>
      )}
      ...
      {isLevelUp && gameState === 'PLAYING' && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-md px-4">
          <div className="text-xs text-red-500 tracking-[0.4em] uppercase mb-2 font-bold" style={{ fontFamily: "'Noto Serif JP', serif" }}>NINJA TECHNIQUE</div>
          <h2
            className="text-5xl md:text-7xl font-black mb-10 text-center tracking-widest uppercase animate-pulse"
            style={{
              fontFamily: "'Noto Serif JP', serif",
              color: '#DC2626',
              textShadow: '0 0 30px rgba(220,38,38,0.8), 0 0 60px rgba(220,38,38,0.3)',
            ...
            {offeredAbilities.map((ability) => {
              ...
              return (
                <button
                  ...
                  className={`group flex-1 flex flex-col items-center p-5 bg-gradient-to-b ${colors.bg} to-black/90 border ${colors.border} rounded-lg ${colors.hoverBg} hover:scale-105 transition-all ${colors.shadow}`}
                >
                  <div className={`w-14 h-14 rounded-lg flex items-center justify-center mb-3 border ${colors.border} bg-black/50`}>
                  ...
                  <div className={`text-xs font-bold uppercase tracking-widest mb-2 ${
                    ability.rarity === 'epic' ? 'text-amber-400' :
                    ability.rarity === 'rare' ? 'text-red-400' : 'text-gray-500'
                  }`}>{ability.rarity}</div>
      ...
      {/* Title Screen */}
      {gameState === 'TITLE' && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
          {/* Ink brush texture overlay */}
          <div
            className="absolute inset-0 pointer-events-none opacity-20"
            style={{
              backgroundImage: 'radial-gradient(ellipse at 30% 20%, rgba(220,38,38,0.15) 0%, transparent 50%), radial-gradient(ellipse at 70% 80%, rgba(220,38,38,0.1) 0%, transparent 50%)',
            }}
          />

          {/* Total Demon Gold Display */}
          <div className="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-black/80 backdrop-blur-sm px-6 py-3 rounded border border-amber-800/40 shadow-[0_0_20px_rgba(217,119,6,0.1)]">
            <Wallet className="w-5 h-5 text-amber-500" />
            <span className="text-amber-400 font-black text-xl tracking-widest">{assets?.credits || 0}</span>
            <span className="text-amber-700 text-xs uppercase tracking-widest">{GAME_CONFIG.ui.currencyLabel}</span>
          </div>

          {/* Title */}
          <div className="text-center mb-10 relative">
            {/* Decorative Japanese line */}
            <div className="w-32 h-px bg-gradient-to-r from-transparent via-red-600 to-transparent mx-auto mb-4" />
            <div className="text-xs text-red-500/80 tracking-[0.6em] uppercase mb-4 font-bold" style={{ fontFamily: "'Noto Serif JP', serif" }}>SHADOWS OF THE DEMON REALM</div>
            <h1
              className="text-5xl sm:text-7xl md:text-9xl font-black uppercase tracking-widest select-none"
              style={{
                fontFamily: "'Noto Serif JP', serif",
                background: 'linear-gradient(180deg, #fef2f2 0%, #dc2626 40%, #7f1d1d 100%)',
            ...
            <div className="text-xs text-gray-600 tracking-[0.4em] uppercase mt-3" style={{ fontFamily: "'Noto Serif JP', serif" }}>SLAY THE DEMON HORDE</div>
            <div className="w-32 h-px bg-gradient-to-r from-transparent via-red-600 to-transparent mx-auto mt-4" />
          </div>

          <div className="flex flex-col items-center gap-4 w-full max-w-xs">
            <button
              onClick={handleStartGame}
              className="group flex items-center justify-center gap-3 w-full py-4 font-black text-xl uppercase tracking-widest rounded transition-all transform hover:-translate-y-1 hover:scale-105 active:scale-95 border"
              style={{
                fontFamily: "'Noto Serif JP', serif",
                background: 'linear-gradient(135deg, rgba(220,38,38,0.2) 0%, rgba(220,38,38,0.05) 100%)',
                borderColor: '#dc2626',
                color: '#dc2626',
                boxShadow: '0 0 30px rgba(220,38,38,0.2), inset 0 0 30px rgba(220,38,38,0.05)',
              }}
            >
            ...
            <button
              onClick={openShop}
              className="flex items-center justify-center gap-3 w-full py-3 font-bold text-base uppercase tracking-widest rounded transition-all transform hover:scale-105 active:scale-95 border"
              style={{
      ...
      {/* Game Over Screen */}
      {gameState === 'GAMEOVER' && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center px-4 bg-red-950/80 backdrop-blur-md">
          {/* Blood splatter decorative overlay */}
          <div className="absolute inset-0 pointer-events-none opacity-20"
            style={{
              backgroundImage: 'radial-gradient(circle at 50% 40%, rgba(127,29,29,0.5) 0%, transparent 60%)',
            }}
          />
          <div className="text-xs text-red-400 tracking-[0.5em] uppercase mb-2 font-bold animate-pulse" style={{ fontFamily: "'Noto Serif JP', serif" }}>THE DEMONS PREVAIL</div>
          <h1
          ...
          <div className="text-gray-500 text-xs tracking-widest mb-8 uppercase" style={{ fontFamily: "'Noto Serif JP', serif" }}>The ninja has been slain</div>

          <div className="w-32 h-px bg-gradient-to-r from-transparent via-amber-700 to-transparent mb-6" />

          <div
            className="flex items-center gap-3 mb-10 px-8 py-4 rounded border font-bold text-lg tracking-widest"
            style={{
          ...
          >
            ...
            <span className="text-amber-700 text-sm">{GAME_CONFIG.ui.currencyLabel} EARNED</span>
          </div>

          <button
            onClick={handleRestartGame}
            className="flex items-center justify-center gap-3 px-8 py-4 font-black text-xl uppercase tracking-widest rounded transition-all transform hover:-translate-y-1 hover:scale-105 active:scale-95 border"
            style={{
              fontFamily: "'Noto Serif JP', serif",
              borderColor: '#dc2626',
              color: '#dc2626',
