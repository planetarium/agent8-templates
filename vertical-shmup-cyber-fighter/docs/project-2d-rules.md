...
이 프로젝트는 **CYBER FIGHTER가 이미 완전히 구현된 상태**입니다.

> ⚠️ **절대 중요**: 이 템플릿은 빈 보일러플레이트가 아닙니다. 수직 스크롤 슈팅 2D 모바일 게임(아케이드 웨이브 기반, 보스전, 에너지 코어 수집)이 이미 완성되어 있습니다.
...
4. **CrossRamp 배포 (필수 — 절대 생략 금지)**:
   ...
   - `.crossramp` `asset_keys`가 `server.js`의 `$asset.mint('<통화키>')` 키와 일치하는지 확인
