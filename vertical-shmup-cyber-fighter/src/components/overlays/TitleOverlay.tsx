...
const TitleOverlay: React.FC<Props> = ({ gameRef }) => {
  ...
  const sbtTokens = stardustBalance !== null ? Math.floor(stardustBalance / 10) : null;
  ...
  return (
    <div className={styles.root}>
      ...
      <header className={styles.header}>
        <div className={styles.badge}>★ NEON ARCADE ★</div>
        <h1 className={styles.title}>
          <span className={styles.titleLine1}>CYBER</span>
          <span className={styles.titleLine2}>FIGHTER</span>
        </h1>
      ...
      <div className={styles.balanceCard}>
        <div className={styles.balanceRow}>
          ...
          <span className={styles.balanceLabel}>ENERGY CORES</span>
        ...
        {sbtTokens !== null && sbtTokens > 0 && (
          <div className={styles.balanceSbt}>
            → <span className={styles.sbtCount}>{sbtTokens}</span> GALAX exchangeable
          </div>
        )}
        {sbtTokens !== null && stardustBalance !== null && stardustBalance > 0 && sbtTokens === 0 && (
          <div className={styles.balanceSbtHint}>
            {10 - (stardustBalance % 10)} more to next GALAX
          </div>
        )}
      </div>
      ...
      {/* Footer hint */}
      <div className={styles.hint}>DRAG TO MOVE · AUTO FIRE</div>
    </div>
  );
};
