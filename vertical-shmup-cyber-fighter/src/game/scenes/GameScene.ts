...
export class GameScene extends Phaser.Scene {
  ...
  private playerBullets: (Phaser.GameObjects.Rectangle | Phaser.GameObjects.Image)[] = [];
  private shootCooldown = 0;
  private shootRate = 180; // ms
  ...
  create() {
    ...
    // Camera flash-in
    this.cameras.main.flash(400, 0, 0, 40);
  }
  ...
  private spawnWave() {
    ...
    if (isBossWave) {
      ...
      this.showWaveBanner(`⚠ BOSS WAVE ${this.waveNumber} ⚠`, '#ff3300');
    } else {
      ...
      this.showWaveBanner(`WAVE ${this.waveNumber}`, '#44aaff');
    }
  }
  ...
  private fireBullet() {
    const b = this.add.image(this.player.x, this.player.y - 36, 'playerBullet').setDepth(4);
    b.setDisplaySize(12, 36);
  ...
  private fireEnemyBullet(enemy: Enemy) {
    ...
    const b = this.add.image(enemy.sprite.x, enemy.sprite.y, 'enemyBullet').setDepth(4);
  ...
  private spawnDrop(x: number, y: number, isBoss = false) {
    // Boss drops always stardust; normal: 80% stardust, 12% crystal, 8% life
    const roll = Math.random();
    const type: 'stardust' | 'crystal' | 'life' = isBoss
      ? 'stardust'
      : roll < 0.80 ? 'stardust' : roll < 0.92 ? 'crystal' : 'life';

    const key = type === 'stardust' ? 'stardust' : 'crystal';
    const tint = type === 'life' ? 0xff4444 : 0xffffff;
    const sprite = this.add.image(x, y, key);
    const size = type === 'stardust' ? 30 : 22;
      ...
      for (let j = this.playerBullets.length - 1; j >= 0; j--) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) < hitR) {
          ...
          if (e.hp <= 0) {
            ...
            // Boss drops 15-20 stardust, normal enemies drop 1-4
    ...
    for (let i = this.drops.length - 1; i >= 0; i--) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) < 36) {
        if (d.type === 'stardust') {
          ...
          this.showFloatText(d.sprite.x, d.sprite.y, '+1 ✦', '#55ddff');
        } else if (d.type === 'crystal') {
          // Crystal gives stardust AND bonus score
          this.stardustCollected += 3;
          this.stardustPending += 3;
          this.score += 150;
          this.showFloatText(d.sprite.x, d.sprite.y, '+3 ✦', '#aaeeff');
        } else {
  ...
  private endGame() {
    ...
    const best = parseInt(localStorage.getItem('starblaster_best') || '0', 10);
    if (this.score > best) localStorage.setItem('starblaster_best', String(this.score));
