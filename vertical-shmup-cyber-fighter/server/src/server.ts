export class Server {
  ...
  async getGameConfig(): Promise<{ exchangeRate: number; tokenSymbol: string; collectibleName: string }> {
    return {
      exchangeRate: 10,      // Must match exchange_rate in .crossramp
      tokenSymbol: 'GALAX',     // On-chain token symbol shown in WalletOverlay
      collectibleName: 'ENERGY CORE', // In-game collectible name shown in WalletOverlay
    };
  }

  // Award energy cores after defeating enemies
  async claimStardust(amount: number): Promise<{ balance: number }> {
    if (amount <= 0 || amount > 1000) throw new Error('Invalid stardust amount');
    await $asset.mint('energy_core', amount);
    const balance = await $asset.get('energy_core');
    return { balance };
  }
  ...
  async getPlayerStats(): Promise<{ bestScore: number; bestWave: number; stardust: number }> {
    const myState = await $global.getMyState();
    const stardust = await $asset.get('energy_core');
    return {
