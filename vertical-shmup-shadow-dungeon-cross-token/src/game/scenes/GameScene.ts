...
export class GameScene extends Phaser.Scene {
  ...
  private soulgemsCollected = 0;
  private soulgemsPending = 0;
  ...
  create() {
    ...
    this.soulgemsCollected = 0;
    this.soulgemsPending = 0;
    ...
    this.joystickBaseRing = this.add.circle(0, 0, this.joystickRadius, 0x8844ff, 0)
      .setDepth(20).setVisible(false);
    this.joystickBaseRing.setStrokeStyle(2, 0x8844ff, 0.35);
    this.joystickHandle = this.add.circle(0, 0, this.joystickHandleRadius, 0x8844ff, 0.25)
      .setDepth(21).setVisible(false);
    this.joystickHandle.setStrokeStyle(2, 0xaa66ff, 0.8);
    ...
    // Camera flash-in
    this.cameras.main.flash(400, 30, 0, 50);
  }
  ...
  private emitHUD() {
    const data: HUDData = {
      ...
      soulgems: this.soulgemsCollected,
      hp: this.playerHP,
      maxHp: this.maxHP,
    };
  ...
  private spawnWave() {
    ...
    if (isBossWave) {
      ...
      this.showWaveBanner(`DEMON LORD - FLOOR ${this.waveNumber}`, '#ff4400');
    } else {
      ...
      this.showWaveBanner(`FLOOR ${this.waveNumber}`, '#aa66ff');
    }
  }
  ...
  private fireBullet() {
    const b = this.add.rectangle(
      this.player.x, this.player.y - 36, 5, 18, 0x9933ff
    ).setDepth(4);
  ...
  private fireEnemyBullet(enemy: Enemy) {
    ...
    const b = this.add.ellipse(enemy.sprite.x, enemy.sprite.y, 10, 14,
      enemy.isBoss ? 0xff4400 : 0xff2222
    ).setDepth(4);
    b.setData('vx', vx);
    b.setData('vy', vy);
  ...
  private spawnDrop(x: number, y: number, isBoss = false) {
    // Boss drops always soulgem; normal: 80% soulgem, 12% crystal, 8% life
    const roll = Math.random();
    const type: 'soulgem' | 'crystal' | 'life' = isBoss
      ? 'soulgem'
      : roll < 0.80 ? 'soulgem' : roll < 0.92 ? 'crystal' : 'life';

    const key = type === 'soulgem' ? 'soulgem' : 'crystal';
    const tint = type === 'life' ? 0xff4444 : 0xffffff;
    const sprite = this.add.image(x, y, key);
    const size = type === 'soulgem' ? 30 : 22;
  ...
  private explodeAt(x: number, y: number, big = false) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const colors = [0x8800ff, 0xaa44ff, 0xff4400, 0x552299];
  ...
  update(time: number, delta: number) {
      ...
      for (let j = this.playerBullets.length - 1; j >= 0; j--) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) < hitR) {
          ...
          if (e.hp <= 0) {
            ...
            // Boss drops 15-20 soulgems, normal enemies drop 1-4
    ...
    for (let i = this.drops.length - 1; i >= 0; i--) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) < 36) {
        if (d.type === 'soulgem') {
          this.soulgemsCollected++;
          this.soulgemsPending++;
          this.showFloatText(d.sprite.x, d.sprite.y, '+1 SOUL', '#bb77ff');
        } else if (d.type === 'crystal') {
          // Crystal gives soulgems AND bonus score
          this.soulgemsCollected += 3;
          this.soulgemsPending += 3;
          this.score += 150;
          this.showFloatText(d.sprite.x, d.sprite.y, '+3 SOUL', '#9944ff');
        } else {
          if (this.playerHP < this.maxHP) {
            this.playerHP++;
          }
          this.showFloatText(d.sprite.x, d.sprite.y, '+LIFE', '#ff6666');
        }
        this.emitHUD();
        d.sprite.destroy();
  ...
  private hitPlayer() {
    ...
    this.emitHUD();
    this.cameras.main.flash(120, 80, 0, 30);
  ...
  private endGame() {
    ...
    if (this.score > best) localStorage.setItem('shadowdungeon_best', String(this.score));

    this.scene.start('GameOverScene', {
      score: this.score,
      wave: this.waveNumber,
      soulgems: this.soulgemsCollected,
      soulgemsPending: this.soulgemsPending,
    });
  }

  getSoulgems() { return this.soulgemsCollected; }
}
