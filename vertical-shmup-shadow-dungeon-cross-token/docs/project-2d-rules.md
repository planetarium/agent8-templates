...
4. Read `server/src/server.ts` → confirm `soulgem` asset key
5. If user specifies a new token/drop name: rename `soulgem` key in `server/src/server.ts`, `GameScene.ts`, and `.crossramp` (3-way sync)
