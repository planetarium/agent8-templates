...
const VARIANT_COLORS: Array<{ emissive: string }> = [
  { emissive: '#c4a265' },  // golden amber
  { emissive: '#6b8f4e' },  // forest green
  { emissive: '#e8d5a3' },  // pale gold
];
...
interface SpiritRuneItemProps {
  loot: LootItem;
  isBeingMined: boolean;
}

const SpiritRuneItem: React.FC<SpiritRuneItemProps> = React.memo(({ loot, isBeingMined }) => {
  const coreModel = useGLTF(Assets.objects.collectible_rune.url);
  ...
  useFrame((_, delta) => {
    ...
    if (matRef.current) {
      ...
      matRef.current.emissiveIntensity = isBeingMined ? 2.0 + pulse * 2.5 : 1.0 + pulse * 1.0;
    }
    if (groupRef.current) {
      groupRef.current.position.y = loot.position.y + 0.5 + Math.sin(timeRef.current * 0.5) * 0.25;
...
SpiritRuneItem.displayName = 'SpiritRuneItem';
...
function buildLootFromSlot(
  ...
  const position = terrainUtil.calculatePositionOnTerrain(terrainData, [sx, sz], 1.0);
  return {
    id: `rune_${idSuffix}`,
    slotIdx,
    position,
...
const LootManager: React.FC<LootManagerProps> = ({ terrainData }) => {
  ...
  const collectCore = useInventoryStore((state) => state.collectCrystal);
  const { connected, server } = useGameServer();
  const { miningTargetId, miningProgress, startMining, setMiningProgress, stopMining } =
    useMiningStore();
  const maxActive = useQualityStore((s) => s.config.maxCrystals);
    ...
    async (id: string) => {
      ...
      if (connected && server) {
        try {
          ...
          console.error('Failed to sync spirit rune collection', err);
        }
      }
    },
    [collectCore, connected, server],
  );
