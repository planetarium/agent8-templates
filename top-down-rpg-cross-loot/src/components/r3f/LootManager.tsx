...
const VARIANT_COLORS: Array<{ emissive: string; glow: string }> = [
  { emissive: '#ff44ff', glow: '#ffccff' }, // pink
  { emissive: '#aa44ff', glow: '#ddaaff' }, // purple
  { emissive: '#ff4488', glow: '#ffccdd' }, // magenta
];
...
const LootManager: React.FC<LootManagerProps> = ({ terrainData }) => {
  ...
  const collectCrystal = useInventoryStore((state) => state.collectCrystal);
  ...
  const handleLootCollected = useCallback(
    ...
    [collectCrystal, connected, server],
  );
