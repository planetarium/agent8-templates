...
5. If user specifies a new token/drop name: rename `credits` key in `server.js`, client code, and `.crossramp` (3-way sync) — NEVER rename function `addGold` (hardcoded in `App.tsx`)
...
- Rewrite or retheme game scenes (TitleScene, MainScene, GameOverScene)
