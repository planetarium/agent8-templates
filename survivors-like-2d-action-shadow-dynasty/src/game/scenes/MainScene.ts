...
export class MainScene extends Phaser.Scene {
  ...
  hitEnemy(projectile: Phaser.Types.Physics.Arcade.GameObjectWithBody, enemy: Phaser.Types.Physics.Arcade.GameObjectWithBody) {
    ...
    this.spawnParticleBurst(en.x, en.y, 0xaa44ff, 8);

    if (!this.pierceEnabled) {
      proj.destroy();
    }
      ...
      if (this.aoeRadius > 0 && this.aoeDamage > 0) {
        this.enemies.getChildren().forEach((other: unknown) => {
          ...
          if (dist <= this.aoeRadius) {
            this.spawnParticleBurst(o.x, o.y, 0x9333ea, 6);
  ...
  collectCoin(_player: Phaser.Types.Physics.Arcade.GameObjectWithBody, coin: Phaser.Types.Physics.Arcade.GameObjectWithBody) {
    ...
    const cx = c.x;
    const cy = c.y;
    this.spawnFloatingText(cx, cy, '+1 SOUL', {
      color: 0xc084fc,
      fontSize: 26,
      shadowColor: 'rgba(100,50,150,0.7)',
      glow: true,
    });
    this.spawnParticleBurst(cx, cy, 0xa855f7, 14);
    c.destroy();
  ...
  collectGem(_player: Phaser.Types.Physics.Arcade.GameObjectWithBody, gem: Phaser.Types.Physics.Arcade.GameObjectWithBody) {
    ...
    const gx = g.x;
    const gy = g.y;
    this.spawnFloatingText(gx, gy, '+1 RUNE', {
      color: 0xf87171,
      fontSize: 26,
      shadowColor: 'rgba(150,30,30,0.6)',
      glow: true,
    });
    this.spawnParticleBurst(gx, gy, 0xdc2626, 14);
    g.destroy();
