...
import {
  Zap, Wallet, Play, RotateCcw, Heart, LogOut, X, Check, Shield, Skull,
} from "lucide-react";
...
const COLOR_CLASSES: Record<string, { border: string; bg: string; hoverBg: string; text: string; shadow: string }> = {
  ...
  green: { border: 'border-emerald-500', bg: 'from-emerald-900/50', hoverBg: 'hover:bg-emerald-900/80', text: 'text-emerald-400', shadow: 'shadow-[0_0_30px_rgba(16,185,129,0.3)]' },
  ...
  cyan: { border: 'border-violet-500', bg: 'from-violet-900/50', hoverBg: 'hover:bg-violet-900/80', text: 'text-violet-400', shadow: 'shadow-[0_0_30px_rgba(139,92,246,0.3)]' },
};
...
function App() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black text-white" style={{ fontFamily: '"Cinzel", "Times New Roman", serif' }}>
      {/* HUD - Playing State */}
      {gameState === 'PLAYING' && (
        <div
        ...
        >
          <div className="absolute top-0 left-0 right-0 p-4 pointer-events-none">
            ...
            <div className="absolute left-4 top-4 flex flex-col gap-2 pointer-events-auto hud-ui z-20">
              <div className="flex items-center gap-1 bg-black/80 backdrop-blur-sm px-4 py-2 rounded-lg border border-purple-500/30 shadow-[0_0_12px_rgba(168,85,247,0.2)]">
                {[...Array(health.max)].map((_, i) => (
                  <Skull
                    key={i}
                    className={`w-6 h-6 transition-all duration-300 ${
                      i < health.current
                        ? 'fill-purple-400 text-purple-400 drop-shadow-[0_0_8px_rgba(168,85,247,0.9)] scale-100'
            ...
            {/* Top Center: Soul Essence */}
            <div className="absolute left-1/2 -translate-x-1/2 top-4 flex flex-col gap-2 items-center pointer-events-auto hud-ui z-20">
              <div className={`flex items-center gap-2 bg-black/80 backdrop-blur-sm px-5 py-2 rounded-lg border border-purple-500/40 shadow-[0_0_15px_rgba(168,85,247,0.25)] ${goldBounce ? 'scale-110' : ''} transition-transform duration-150`}>
                <Skull className="w-5 h-5 text-purple-400" />
                <span className="text-purple-300 font-bold text-xl tracking-widest">{sessionGold}</span>
                <span className="text-purple-600 text-xs uppercase tracking-widest">SOULS</span>
              </div>
            </div>
            ...
            <div className="absolute right-4 top-4 flex flex-col gap-2 pointer-events-auto hud-ui z-20">
              <button
                ...
                className="flex items-center justify-center bg-black/80 hover:bg-red-900/80 backdrop-blur-sm p-3 rounded-lg border border-purple-500/20 hover:border-red-500/50 transition-colors shadow-[0_0_10px_rgba(0,0,0,0.5)]"
              >
          ...
          <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 w-72 md:w-[560px] pointer-events-auto hud-ui z-20 ${xpPulse ? 'scale-105' : ''} transition-transform duration-150`}>
            <div className="flex items-center justify-between text-xs font-bold mb-2 px-1">
              <span className="text-red-400 tracking-widest uppercase">CIRCLE {xp.level}</span>
              <span className="text-red-300/70">{xp.current} / {xp.max} RUNES</span>
            </div>
            <div className="w-full bg-black/60 backdrop-blur-md rounded-sm h-3 border border-red-900/50 overflow-hidden shadow-[0_0_10px_rgba(0,0,0,0.5)]">
              <div
          ...
          {joystick?.active && !isLevelUp && (
            ...
            >
              {/* Base ring */}
              <div className="absolute inset-0 rounded-full border-2 border-purple-400/40 bg-purple-900/10 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]" />
              {/* Stick */}
              <div
      ...
      {isQuitModalOpen && gameState === 'PLAYING' && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-md px-4 pointer-events-auto hud-ui">
          <div className="bg-gray-950 border-2 border-purple-500/40 rounded-2xl p-8 max-w-sm w-full flex flex-col items-center shadow-[0_0_60px_rgba(168,85,247,0.15)]">
            {/* Decorative top bar */}
            <div className="w-full h-px bg-gradient-to-r from-transparent via-purple-500 to-transparent mb-6" />
            <Skull className="w-14 h-14 text-red-400 mb-4 drop-shadow-[0_0_12px_rgba(239,68,68,0.7)]" />
            ...
            <div className="flex gap-3 w-full">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-red-800 hover:bg-red-700 text-white font-bold rounded-lg transition-colors border border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.3)]"
              >
                <Check className="w-4 h-4" /> RETREAT
              </button>
            </div>
            <div className="w-full h-px bg-gradient-to-r from-transparent via-purple-500 to-transparent mt-6" />
          </div>
        </div>
      )}
      ...
      {isLevelUp && gameState === 'PLAYING' && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-md px-4">
          <div className="text-xs text-red-400 tracking-[0.4em] uppercase mb-2 font-bold">FORBIDDEN KNOWLEDGE</div>
          <h2
            className="text-4xl md:text-7xl font-black mb-10 text-center tracking-widest uppercase animate-pulse"
            style={{
              color: '#A855F7',
              ...
              return (
                ...
                >
                  ...
                  <div className={`text-xs font-bold uppercase tracking-widest mb-2 ${
                    ...
                    ability.rarity === 'rare' ? 'text-purple-400' : 'text-gray-400'
                  }`}>{ability.rarity}</div>
      ...
      {gameState === 'TITLE' && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
          ...
          {/* Total Souls Display */}
          <div className="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-black/80 backdrop-blur-sm px-6 py-3 rounded-lg border border-purple-500/30 shadow-[0_0_20px_rgba(168,85,247,0.15)]">
            <Wallet className="w-5 h-5 text-purple-400" />
            <span className="text-purple-300 font-black text-xl tracking-widest">{assets?.credits || 0}</span>
            <span className="text-purple-600 text-xs uppercase tracking-widest">SOULS</span>
          </div>

          {/* Title */}
          <div className="text-center mb-10 relative">
            <div className="text-xs text-red-400 tracking-[0.5em] uppercase mb-3 font-bold">THE UNDYING THRONE</div>
            <h1
              className="text-5xl sm:text-7xl md:text-9xl font-black uppercase tracking-widest select-none"
              style={{
                background: 'linear-gradient(180deg, #e9d5ff 0%, #a855f7 40%, #7f1d1d 100%)',
            ...
            <div className="text-xs text-gray-500 tracking-[0.4em] uppercase mt-3">COMMAND THE LEGIONS OF THE DEAD</div>
          </div>
      ...
      {gameState === 'GAMEOVER' && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center px-4 bg-red-950/80 backdrop-blur-md">
          <div className="text-xs text-amber-400 tracking-[0.5em] uppercase mb-2 font-bold animate-pulse">THE LIGHT PREVAILS</div>
          <h1
          ...
          <div className="text-gray-400 text-xs tracking-widest mb-8 uppercase">Your dark reign has ended</div>
          ...
          >
            ...
            <span className="text-purple-500 text-sm">SOULS HARVESTED</span>
          </div>
