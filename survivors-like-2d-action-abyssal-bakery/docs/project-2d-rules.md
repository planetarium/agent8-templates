# ABYSSAL BAKERY — 완성된 게임 템플릿 AI 에이전트 지침
...
이 프로젝트는 **ABYSSAL BAKERY가 이미 완전히 구현된 상태**입니다.

> ⚠️ **절대 중요**: 이 템플릿은 빈 보일러플레이트가 아닙니다. 게임이 이미 완성되어 있습니다.
> - 세계관: 음식/제과 세계 (요리사 vs 악성 빵 생물들)
> - 플레이어: Combat Baker (전투 제빵사)
> - 적: Moldy Croissant, 바게트, 도넛 등 음식 몬스터
> - 통화: Magic Flour (마법 밀가루)
> - UI 테마: 핑크(#ec4899) 기반

## 섹션 0-1 — 첫 프롬프트 금지 사항 (CRITICAL)

> 🚫 첫 프롬프트에서 아래 작업들을 **절대 수행하지 마십시오**:
1. 에셋 생성 금지 — 이미 assets.json에 완성된 에셋 URL 존재
2. UI 재설계 금지 — App.tsx가 이미 완성된 상태
3. config 파일 전면 교체 금지 — gameConfig.ts, enemyTypes.ts, abilities.ts, waves.ts 모두 완성됨
4. 사용자 요청 없이 코드 수정 금지

## 섹션 0-2 — 첫 프롬프트 필수 행동

1. 기존 코드 파악 (PROJECT/Structure.md 확인)
2. 사용자에게 현재 구현 상태 보고
3. 변경 사항 확인
4. **CrossRamp 배포 (필수 — 절대 생략 금지)**:
   - `read_gameserver_sdk_v2` 호출
   - `read_gameserver_sdk_crossramp` 호출
   - `.crossramp` `asset_keys: ["magic_flour"]` 확인

---

## 섹션 1 — 보일러플레이트 목적 (커스터마이징 필요 시 참고)

이 코드베이스는 **뱀파이어 서바이버류 2D 모바일 게임** 구조를 기반으로 합니다. 사용자가 새 컨셉으로 재창조를 요청하는 경우에만 섹션 2 이후 규칙을 따릅니다.
