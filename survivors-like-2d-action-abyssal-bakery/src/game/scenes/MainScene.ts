...
export class MainScene extends Phaser.Scene {
  ...
  private handleSelectAbility = (e: Event) => {
    ...
    } else if (eff.type === 'dough_trap') {
      this.doughTrapSlowFactor = eff.slowFactor;
    }
  ...
  hitEnemy(projectile: Phaser.Types.Physics.Arcade.GameObjectWithBody, enemy: Phaser.Types.Physics.Arcade.GameObjectWithBody) {
    ...
    if (this.doughTrapSlowFactor > 0 && en.getData('slowed') !== true) {
      en.setData('slowed', true);
      en.setData('speedMultiplier', this.doughTrapSlowFactor);
    }
    ...
    this.time.delayedCall(100, () => {
      if (en.active) {
        ...
        else if (en.getData('slowed') === true) en.setTint(0xffaacc);
        else en.clearTint();
      }
    });
