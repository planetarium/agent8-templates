# Structure — basic-3d-quarterview (Incanto)
...
physics auto, keyboard, `pointer: { lockOnClick: false }` — the camera is
fixed, renderer, start) → button key-injection → `window.game`.
