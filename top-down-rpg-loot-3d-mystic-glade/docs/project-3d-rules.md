...
이 프로젝트는 **Mystic Glade가 이미 완전히 구현된 상태**입니다.

> ⚠️ **절대 중요**: 이 템플릿은 빈 보일러플레이트가 아닙니다. 마법 숲에서 요정 씨앗을 채집하고 CROSS 토큰으로 교환하는 3D 탑다운 오픈월드 게임이 이미 완성되어 있습니다.
...
4. **CrossRamp 배포 (필수 — 절대 생략 금지)**:
   ...
   - `.crossramp` `asset_keys`가 `server.ts`의 `$asset.mint('fairy_seed')` 키와 일치하는지 확인
...
| 수집품 이름 표시 | `InventoryHUD.tsx` 라벨 + 아이콘 SVG | "Fairy Seeds" 텍스트, SVG 교체 |
| 채집 진행 라벨 | `MiningProgressUI.tsx` | 텍스트, 색상 |
...
| **지형 색상** | `GameEnvironment.tsx` → `<Terrain color="...">` | 바닥/지형 기본 색상 |
| **지형 시드** | `GameEnvironment.tsx` → `<Terrain seed="...">` | 지형 모양 결정 |
| 서버 에셋 키 | `server/src/server.ts` → `$asset.mint('fairy_seed')` | 수집품 키 변경 |
| InventoryHUD 에셋 키 | `InventoryHUD.tsx` → `assets?.fairy_seed` | 서버 키와 동일하게 |
| TitleScene 아이콘/색상 | `TitleScene.tsx` | SVG 아이콘, 그라데이션, 파티클 |
| InventoryHUD 아이콘/색상 | `InventoryHUD.tsx` | SVG 아이콘, 보더, 글로우 색상 |
| 교환 버튼 텍스트 | `TitleScene.tsx`, `InventoryHUD.tsx` | "⇄ EXCHANGE SEEDS" |
